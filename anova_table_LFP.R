...
# ---- Step 6: Tidy Each Model Result ----
# Tidy the ANOVA result manually by extracting the relevant terms
anova_tidy <- broom::tidy(anova_lfp_repeated$`Error: Subj:Trial_Type`)

# Tidy the LME model, filtering only fixed effects
lme_tidy <- broom.mixed::tidy(lme_model) %>%
  filter(effect == "fixed")  # Only include fixed effects (omit random effects)

# Tidy the ANCOVA results for HAM-D and HIT-6
ancova_hamd_tidy <- broom::tidy(ancova_hamd)
ancova_hit6_tidy <- broom::tidy(ancova_hit6)

# Convert Post-hoc Tukey test result to a tidy data frame
posthoc_tidy <- as.data.frame(posthoc_summary$contrasts) %>%
  rename(term = contrast, estimate = estimate, std.error = SE, p.value = p.value)

# ---- Step 7: Combine the Tidied Results ----
# Combine all into a single data frame with a new "Analysis" column
...
# ---- Step 8: Select the Necessary Columns for Consistency ----
